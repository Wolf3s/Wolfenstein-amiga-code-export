...
#elif defined(GP2X_940) || defined (__AMIGA__)
...
#if defined(__AMIGA__) && !defined(USE_HALFWIDTH)
        viewsize = 18; // divisible by 32
#else
...
#ifdef USE_AUTOMAP
    word mapvisnum = 0;
	// step one: count the visible tiles
    for(i=0;i<MAPSIZE;i++)
    {
        for(int j=0;j<MAPSIZE;j++)
        {
            if (mapvis[j][i]) mapvisnum++;
        }
    }
	// step two save the coordinates
	fwrite (&mapvisnum,sizeof(mapvisnum),1,file);
    for(i=0;i<MAPSIZE;i++)
    {
        for(int j=0;j<MAPSIZE;j++)
        {
            if (mapvis[j][i])
			{
				byte mx = j;
				byte my = i;
				fwrite (&mx,sizeof(mx),1,file);
				fwrite (&my,sizeof(my),1,file);
			}
        }
    }
#endif
...
boolean LoadTheGame(FILE *file,int x,int y)
{
    ...
    word mapvisnum = 0xFFFF;
    size_t count = fread (&mapvisnum,sizeof(mapvisnum),1,file);
	if (count > 0 && mapvisnum < 0xFFFF)
	{
		for (i = 0; i < mapvisnum; i++)
		{
			byte mx, my;
			fread (&mx,sizeof(mx),1,file);
			fread (&my,sizeof(my),1,file);
			mapvis[mx][my] = 1;
		}
	}
#endif
...
void BuildTables (void)
{
	...
	for (int i = 1; i < MAXSCALEHEIGHT*2; i++)
	{
		//scalediv[i] = (64<<8) / (fixed)i;
...
void CalcProjection (int32_t focal)
{
    ...
    //
#ifdef __AMIGA__
    scale = halfview*(FOCALLENGTH+MINDIST)/(VIEWGLOBAL/2);
#ifdef USE_HALFWIDTH
	scale *= 2;
#endif
#else
	...
	for (i=1;i<MAXDIST;i++)
	{
		heightnumdiv[i] = heightnumerator / i;
	}
#ifdef USE_HALFWIDTH
	scale /= 2;
#endif
...
#ifdef __AMIGA__
        intang = pixeltang[MAXVIEWWIDTH*i/viewwidth];
#else
...
#ifdef __AMIGA__
    byte *signon = calloc(320*200, 1);
    CHECKMALLOCRESULT(signon);
    char fname[13] = "signon.";
...
#endif
#if 0
    /*FILE **/file = fopen ("signon.bin", "wb");
            fwrite (signon, 320*200, 1, file);
            fclose (file);
#endif
}
...
boolean SetViewSize (unsigned width, unsigned height)
{
    ...
    {
#ifdef USE_HALFWIDTH
        viewscreenx = (screenWidth-viewwidth*2) / 2;
#else
...
#ifdef __AMIGA__
	VL_ReAllocChunkyBuffer();
	//SetupScaling (viewwidth*3/2);
#endif
...
#ifdef USE_HALFWIDTH
		viewwidth /= 2;
		viewwidth = viewwidth&~15; // round down to a multiple of 16, like SetViewSize
#endif
...
#ifdef __AMIGA__
			boolean remapped = VL_RemapBufferCustomEHB(grsegs[TITLE1PIC], pictable[TITLE1PIC - STARTPICS].width * pictable[TITLE1PIC - STARTPICS].height, pal);
#endif
...
#ifdef __AMIGA__
			VL_RemapBufferCustomEHB(grsegs[TITLE2PIC], pictable[TITLE2PIC - STARTPICS].width * pictable[TITLE2PIC - STARTPICS].height, pal);
#endif
...
#ifdef __AMIGA__
			if(param_joystickindex < -1 || param_joystickindex > 1)
			{
				printf("The joystick index must be between -1 and 1!\n");
                hasError = true;
			}
#endif
        }
...
#ifdef __AMIGA__
            "Amiga conversion by Szilard Biro\n"
#endif
...
*/
#ifdef __AMIGA__
// we don't need the default CTRL-C handler
