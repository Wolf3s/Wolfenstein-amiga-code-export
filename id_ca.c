...
#ifdef __AMIGA__
	size_t count = lengthof(grstarts) * 3;
    byte *data = malloc(count);
	CHECKMALLOCRESULT(data);
	read(handle, data, count);
#else
...
//
	...
	for (int i = 0; i < NUMPICS; ++i)
	{
		...
		//printf("%s %d: %d x %d\n", __FUNCTION__, i, pictable[i].width, pictable[i].height);
	}
#endif
    free(compseg);
}
...
#ifdef __AMIGA__
	tinf->RLEWtag = SWAP16LE(tinf->RLEWtag);
	for (int i = 0; i < sizeof(tinf->headeroffsets) / sizeof(int32_t); i++)
		tinf->headeroffsets[i] = SWAP32LE(tinf->headeroffsets[i]);
#endif
    close(handle);
...
//
    for (i=0;i<NUMMAPS;i++)
    {
		...
		for (int j = 0; j < sizeof(mapheaderseg[i]->planelength) / sizeof(word); j++)
			mapheaderseg[i]->planelength[j] = SWAP16LE(mapheaderseg[i]->planelength[j]);

		for (int j = 0; j < sizeof(mapheaderseg[i]->planestart) / sizeof(int32_t); j++)
			mapheaderseg[i]->planestart[j] = SWAP32LE(mapheaderseg[i]->planestart[j]);
...
{
#ifdef __AMIGA__
	int32_t pos = SWAP32LE(audiostarts[chunk]);
	int32_t size = SWAP32LE(audiostarts[chunk+1]) - pos;
#else
...
#ifdef __AMIGA__
static void CAL_DemungePic(byte *source, int width, int height)
{
	byte *pic = (byte *) malloc(width*height);
	CHECKMALLOCRESULT(pic);
	memcpy (pic,source,width*height);
	for(int j=0; j<height; j++)
	{
		for(int i=0; i<width; i++)
		{
			source[j*width+i] = pic[(j*(width>>2)+(i>>2))+(i&3)*(width>>2)*height];
		}
	}
	free(pic);
}
...
#define MASKBLOCK       128
	...
	if (((chunk >= STARTFONT) && (chunk < STARTFONT+NUMFONT)) || ((chunk >= STARTFONTM) && (chunk < STARTFONTM+NUMFONTM)))
	{
		...
		for (int i = 0; i < (int)(sizeof(font->location)/sizeof(*(font->location))); ++i)
		{
			...
			//printf("char %d location %d width %d\n", i, font->location[i], font->width[i]);
		}
	}
	else if (chunk >= STARTPICS && chunk < STARTPICS+NUMPICS)
	{
		int picnum = chunk - STARTPICS;
		CAL_DemungePic(grsegs[chunk], pictable[picnum].width, pictable[picnum].height);
		//printf("chunk %d exp %d picnum %d w %d h %d\n", chunk, expanded, picnum, pictable[picnum].width, pictable[picnum].height);
		//printf("pic %p(%p)\n", pic, grsegs[chunk]);
	}
...
// Sprites need to have shifts made and various other junk
//
    byte *pic = (byte *) malloc(64000); // TODO get rid of the temp buffer
    CHECKMALLOCRESULT(pic);
    CAL_HuffExpand((byte *) source, pic, expanded, grhuffman);

#ifdef __AMIGA__
	CAL_DemungePic(pic, 320, 200);
...
#ifdef __AMIGA__
		uint16_t *rlewunsignedptr = source;
