...
SDL_Surface     *latchpics[NUMLATCHPICS]; // freed in VL_Shutdown
...
UWORD *fontTemplate[NUMFONT]; // freed in VL_Shutdown


//==========================================================================

static void MakeFontTemplate(void)
{
	// TODO: this will only work with font width <= 16
	//UWORD *template;
	fontstruct *font;

	font = (fontstruct *) grsegs[STARTFONT+fontnumber];

	fontTemplate[fontnumber] = AllocVec(sizeof(UWORD) * font->height * 256, MEMF_CHIP | MEMF_CLEAR);
	if (!fontTemplate[fontnumber])
	{
		return;
	}

	//template = fontTemplate[fontnumber];
	UWORD *ptr = fontTemplate[fontnumber];
	...
	for (int currChar = 0; currChar < 256; currChar++)
	{
		...
		currCharFontPtr = ((byte *)font)+font->location[currChar];
		for (currCharPixY = 0; currCharPixY < font->height; ++currCharPixY)
		{
			for (currCharPixX = 0; currCharPixX < font->width[currChar]; ++currCharPixX, ++currCharFontPtr)
			{
				//*ptr |= *currCharFontPtr << (15 - currCharPixX);
				if (*currCharFontPtr)
				{
					*ptr |= 1 << (15 - currCharPixX);
				}
			}
#ifdef __AROS__
			*ptr = AROS_BE2WORD(*ptr);
#endif
			ptr++;
		}
	}
...
void VWB_DrawPropString(const char* string)
{
	...
	if (!fontTemplate[fontnumber])
	{
		MakeFontTemplate(/*font, fontTemplate[fontnumber]*/);
	}
	if (!fontTemplate[fontnumber])
	{
		Quit("Can't create the font template!");
	}

	UWORD *template = fontTemplate[fontnumber];
...
#endif
	byte color = VL_RemapColorEHB(fontcolor);
	//color = fontcolor;
	SetABPenDrMd(rp, color, 0, JAM1);
	while ((ch = (byte)*string++)!=0)
	{
		...
		BltTemplate((PLANEPTR)&template[ch * height], 0, 2, rp, px, py, width, height);
		px += width;
	}
#else
	...
	while ((ch = (byte)*string++)!=0)
	{
		...
		//bug("%s %d %d - %d %d\n", __FUNCTION__, px, py, width, height);
		...
		while (width--)
		{
			for(int i=0;i<height;i++)
			{
				if(source[i*step])
                {
					...
					// TODO this will be super-slow, convert fonts to templates
...
{
#ifdef __AMIGA__
	// TODO check before changing the buffer?
	//ChangeScreenBuffer(screen, sbuf[displayofs]);
#else
...
#ifdef __AMIGA__
	unsigned width,height;
	unsigned latchidx = 2+picnum-LATCHPICS_LUMP_START;
	unsigned tabidx = picnum-STARTPICS;
	width = pictable[tabidx].width;
	height = pictable[tabidx].height;

	//bug("%s(%u,%u,%u) %u x %u %u %u\n", __FUNCTION__, x, y, picnum, width, height, latchidx,tabidx);
	VL_LatchToScreen (latchpics[latchidx], 0, 0, width, height, x*8, y);
#else
...
//
	...
	surf = VL_CreateSurface(8*8, ((NUMTILE8 + 7) / 8) * 8);
    if(surf == NULL)
    {
...
//
	...
	for (i=start;i<=end;i++)
	{
		...
		surf = VL_CreateSurface(width, height);
        if(surf == NULL)
        {
    ...
    unsigned width, unsigned height, unsigned frames, boolean abortable)
{
	...
	//printf("%s bufferofs %d displayofs %d\n", __FUNCTION__, bufferofs, displayofs);
	// swap bufferofs and displayofs so we plot from the buffer to the screen
	displayofs = bufferofs;
	byte *chunky = malloc(width*height);
	if (chunky) VL_ScreenToMem(chunky, width, height, x1, y1);
	bufferofs ^= 1;
    ...
    do
    {
...
#ifdef __AMIGA__
			// VL_ScreenToScreen
			// switch to the new displayofs immediately
        ...
        //for(int i = first; i < 2; i++)
        {
            for(unsigned p = 0; p < pixperframe; p++)
            {
				...
				if (chunky)
				{
					col = chunky[x+y*width];
				}
				else
				{
					bufferofs ^= 1;
					col = VL_GetPixel(px, py);
					bufferofs ^= 1;
				}
				VL_Plot(px, py, col);
#else
