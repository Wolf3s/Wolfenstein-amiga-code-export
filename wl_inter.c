...
#ifdef __AMIGA__
	CA_CacheGrChunk (screen);
	boolean remapped = VL_RemapBufferCustomEHB(grsegs[screen], pictable[screen - STARTPICS].width * pictable[screen - STARTPICS].height, pal);
	VWB_DrawPic (0, 0, screen);
	UNCACHEGRCHUNK (screen);
	if (remapped)
...
#ifdef __AMIGA__
	CA_CacheGrChunk (ENDSCREEN3PIC);
	boolean remapped = VL_RemapBufferCustomEHB(grsegs[ENDSCREEN3PIC], pictable[ENDSCREEN3PIC - STARTPICS].width * pictable[ENDSCREEN3PIC - STARTPICS].height, pal);
	VWB_DrawPic (0, 0, ENDSCREEN3PIC);
	UNCACHEGRCHUNK (ENDSCREEN3PIC);
	if (remapped)
		VL_FadeIn(0,255,gamepal,30);
	else
#endif
...
#ifdef __AMIGA__
		char *s = parTimes[gamestate.episode * 10 + mapon].timestr;
		int partime = atoi(s)*60 + atoi(s+3);
		//printf("%s %s -> %02d:%02d (%d)\n", __FUNCTION__, s, partime / 60, partime % 60, partime);
        if (gamestate.TimeCount < partime * 70)
            timeleft = partime - sec;
#else
...
static byte gotpage[PAGE_SIZE(666)]; // Wolf3D 663, Spear 666
...
PreloadGraphics (void)
{
	...
	for (int16_t y=0;y<mapheight;y++)
		for (int16_t x=0;x<mapwidth;x++)
		{
			...
			{
				//printf("%s x %2d y %2d tile %2d horizwall %d (%p) vertwall %d (%p)\n", __FUNCTION__, x, y, tile, horizwall[tile], PM_GetPageAddress(horizwall[tile]), vertwall[tile], PM_GetPageAddress(vertwall[tile]));
...
#define DOORWALL	(PMSpriteStart-8)
				...
				uint16_t doorjam = DOORWALL+2;
				if (doorobjlist[doornum].vertical)
					doorpage++;
				else
					doorjam++;

				//printf("%s x %2d y %2d door type %2d doorpage %d (%p) doorjam %d (%p)\n", __FUNCTION__, x, y, type, doorpage, PM_GetPageAddress(doorpage), doorjam, PM_GetPageAddress(doorjam));
...
#ifndef SPEAR
			if (*(mapsegs[1]+(x<<mapshift)+y) == EXITTILE)
			{
				//printf("%s exit tile at %d %d\n", __FUNCTION__, x, y);
				PreloadSpriteRange(SPR_BJ_W1, SPR_BJ_JUMP4);
				PreloadSound(YEAHSND);
			}
#endif
		}
...
#ifdef SPEAR
			case bo_25clip:
				PreloadSound(GETAMMOBOXSND);
				break;
#endif
			case bo_machinegun:
				PreloadSound(GETMACHINESND);
				break;
			case bo_chaingun:
				PreloadSound(GETGATLINGSND);
				break;
			case bo_fullheal:
				PreloadSound(BONUS1UPSND);
				break;
			case bo_food:
	...
	for (objtype *obj = player->next;obj;obj=obj->next)
	{
		...
		switch (obj->obclass)
		{
			case playerobj:
				...
				break;
			case inertobj:
				PreloadSprite(SPR_GRD_DEAD); // SpawnDeadGuard
				//PreloadSpriteRange(SPR_SMOKE_1, SPR_SMOKE_4); // T_GiftThrow A_Smoke
#ifdef SPEAR
				//PreloadSpriteRange(SPR_HSMOKE_1, SPR_HSMOKE_4); // T_Projectile, A_Smoke
#endif
				break;
			case guardobj:
				PreloadSpriteRange(SPR_GRD_S_1, SPR_GRD_SHOOT3);
				PreloadSprite(SPR_STAT_26); // bo_clip2
				PreloadSound(DEATHSCREAM1SND);
				PreloadSound(DEATHSCREAM2SND);
				PreloadSound(DEATHSCREAM3SND);
				PreloadSound(DEATHSCREAM4SND);
				PreloadSound(DEATHSCREAM5SND);
				PreloadSound(DEATHSCREAM7SND);
				PreloadSound(DEATHSCREAM8SND);
				PreloadSound(DEATHSCREAM9SND);
				PreloadSound(HALTSND);
				PreloadSound(NAZIFIRESND);
				break;
			case officerobj:
				PreloadSpriteRange(SPR_OFC_S_1, SPR_OFC_SHOOT3);
				PreloadSprite(SPR_STAT_26); // bo_clip2
				PreloadSound(NEINSOVASSND);
				PreloadSound(SPIONSND);
				PreloadSound(NAZIFIRESND);
				break;
			case ssobj:
				PreloadSpriteRange(SPR_SS_S_1, SPR_SS_SHOOT3);
				if (gamestate.bestweapon < wp_machinegun)
					PreloadSprite(SPR_STAT_27); // bo_machinegun
				else
					PreloadSprite(SPR_STAT_26); // bo_clip2
				PreloadSound(SSFIRESND);
				PreloadSound(LEBENSND);
				PreloadSound(SCHUTZADSND);
				break;
			case dogobj:
				PreloadSpriteRange(SPR_DOG_W1_1, SPR_DOG_JUMP3);
				PreloadSound(DOGATTACKSND);
				PreloadSound(DOGDEATHSND);
				PreloadSound(DOGBARKSND);
				break;
			case mutantobj:
				PreloadSpriteRange(SPR_MUT_S_1, SPR_MUT_SHOOT4);
				PreloadSprite(SPR_STAT_26); // bo_clip2
				PreloadSound(DEATHSCREAM6SND);
				PreloadSound(NAZIFIRESND);
				break;
#ifndef SPEAR
			case bossobj:
				PreloadSpriteRange(SPR_BOSS_W1, SPR_BOSS_DIE3);
				PreloadSprite(SPR_STAT_20); // bo_key1
				PreloadSound(MUTTISND);
				PreloadSound(GUTENTAGSND);
				PreloadSound(BOSSFIRESND);
				break;
			case schabbobj:
				PreloadSpriteRange(SPR_SCHABB_W1, SPR_SCHABB_DEAD);
				PreloadSound(MEINGOTTSND);
				PreloadSound(SCHABBSHASND);
				PreloadSound(SCHABBSTHROWSND);
			// fallthrough
			case needleobj:
				PreloadSpriteRange(SPR_HYPO1, SPR_HYPO4);
				break;
			case fakeobj:
				PreloadSpriteRange(SPR_FAKE_W1, SPR_FAKE_DEAD);
				PreloadSound(HITLERHASND);
				PreloadSound(TOT_HUNDSND);
				PreloadSound(FLAMETHROWERSND);
			// fallthrough
			case fireobj:
				PreloadSpriteRange(SPR_FIRE1, SPR_FIRE2);
				break;
			case mechahitlerobj:
				PreloadSpriteRange(SPR_MECHA_W1, SPR_MECHA_DIE3);
				PreloadSound(SCHEISTSND);
				PreloadSound(DIESND);
				PreloadSound(BOSSFIRESND);
				PreloadSound(MECHSTEPSND);
			// fallthrough
			case realhitlerobj:
				PreloadSpriteRange(SPR_HITLER_W1, SPR_HITLER_DIE7);
				PreloadSound(EVASND);
				PreloadSound(SLURPIESND);
				PreloadSound(DIESND);
				PreloadSound(BOSSFIRESND);
			...
			case bjobj:
				...
				*/
			case ghostobj:
				PreloadSpriteRange(SPR_BLINKY_W1, SPR_INKY_W2);
				break;
			case gretelobj:
				PreloadSpriteRange(SPR_GRETEL_W1, SPR_GRETEL_DIE3);
				PreloadSprite(SPR_STAT_20); // bo_key1
				PreloadSound(MEINSND);
				PreloadSound(KEINSND);
				PreloadSound(NAZIFIRESND);
				break;
			case giftobj:
				PreloadSpriteRange(SPR_GIFT_W1, SPR_GIFT_DEAD);
				PreloadSound(DONNERSND);
				PreloadSound(EINESND);
				PreloadSound(MISSILEFIRESND);
			// fallthrough
			case rocketobj:
				PreloadSpriteRange(SPR_ROCKET_1, SPR_BOOM_3);
				PreloadSound(MISSILEHITSND);
				break;
			case fatobj:
				PreloadSpriteRange(SPR_FAT_W1, SPR_FAT_DEAD);
				PreloadSpriteRange(SPR_SMOKE_1, SPR_SMOKE_4); // also uses T_GiftThrow and s_rocket
				PreloadSound(ROSESND);
				PreloadSound(ERLAUBENSND);
				PreloadSound(MISSILEFIRESND);
				break;
#else
			case spectreobj:
				PreloadSpriteRange(SPR_SPECTRE_W1, SPR_SPECTRE_F4);
				PreloadSound(GHOSTFADESND);
				PreloadSound(GHOSTSIGHTSND);
				break;
			case angelobj:
				PreloadSpriteRange(SPR_ANGEL_W1, SPR_ANGEL_DEAD);
				PreloadSound(ANGELDEATHSND);
				PreloadSound(SLURPIESND);
				PreloadSound(ANGELTIREDSND);
				PreloadSound(ANGELSIGHTSND);
				PreloadSound(ANGELFIRESND);
				PreloadSound(NAZIFIRESND);
			// fallthrough
			case sparkobj:
				PreloadSpriteRange(SPR_SPARK1, SPR_SPARK4);
				break;
			case transobj:
				PreloadSpriteRange(SPR_TRANS_W1, SPR_TRANS_DIE3);
				PreloadSprite(SPR_STAT_20); // bo_key1
				PreloadSound(TRANSDEATHSND);
				PreloadSound(TRANSSIGHTSND);
				PreloadSound(NAZIFIRESND);
				break;
			case uberobj:
				PreloadSpriteRange(SPR_UBER_W1, SPR_UBER_DEAD);
				PreloadSprite(SPR_STAT_20); // bo_key1
				PreloadSound(UBERDEATHSND);
				PreloadSound(NAZIFIRESND);
				break;
			case willobj:
				PreloadSpriteRange(SPR_WILL_W1, SPR_WILL_DEAD);
				PreloadSprite(SPR_STAT_20); // bo_key1
				PreloadSound(WILHELMDEATHSND);
				PreloadSound(WILHELMSIGHTSND);
				PreloadSound(MISSILEFIRESND);
				PreloadSound(NAZIFIRESND);
			// fallthrough
			case rocketobj:
				...
				break;
			case deathobj:
				PreloadSpriteRange(SPR_DEATH_W1, SPR_DEATH_DEAD);
				PreloadSprite(SPR_STAT_20); // bo_key1
				PreloadSound(KNIGHTDEATHSND);
				PreloadSound(KNIGHTSIGHTSND);
				PreloadSound(KNIGHTMISSILESND);
				PreloadSound(NAZIFIRESND);
			// fallthrough
			case hrocketobj:
				PreloadSpriteRange(SPR_HROCKET_1, SPR_HBOOM_3);
				PreloadSound(MISSILEHITSND);
				break;
#endif
			default:
	...
	PreloadSound(ATKKNIFESND);
	PreloadSpriteRange(SPR_PISTOLREADY, SPR_PISTOLATK4);
	PreloadSound(ATKPISTOLSND);
	PreloadSpriteRange(SPR_MACHINEGUNREADY, SPR_MACHINEGUNATK4);
	PreloadSound(ATKMACHINEGUNSND);
	PreloadSpriteRange(SPR_CHAINREADY, SPR_CHAINATK4);
	PreloadSound(ATKGATLINGSND);

	// misc sfx
	PreloadSound(BONUS1UPSND);
	PreloadSound(HITWALLSND);
	PreloadSound(DONOTHINGSND);
	PreloadSound(NAZIFIRESND); // this a generic sound used by many enemies
#ifndef SPEAR
	if (mapon==9)
#else
	if (mapon==18 || mapon==19)
#endif
	{
		PreloadSound(DEATHSCREAM6SND);
	}
	...
	{
		boolean haspage = PMPages[i] != NULL;
