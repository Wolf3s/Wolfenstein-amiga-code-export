...
static const BE_GameFileDetails_T gt14[] =
{
	{"AUDIOHED", 1156, 0x2F3BCB70},
	{"AUDIOT", 320209, 0xC79EFD4C},
	{"GAMEMAPS", 150652, 0xADA5C827},
	{"MAPHEAD", 402, 0x015803E9},
	{"VGADICT", 1024, 0xAA22054E},
	{"VGAGRAPH", 275774, 0xA4B1DE1D},
	{"VGAHEAD", 450, 0xAB2B44D3},
	{"VSWAP", 1544376, 0xE610C664},
	{NULL, 0, 0}
};

static const BE_GameFileDetails_T apogee11[] =
{
	{"AUDIOHED", 1156, 0x056AC3DB},
	{"AUDIOT", 317049, 0x1585D305},
	{"GAMEMAPS", 150746, 0xDB7B0962},
	{"MAPHEAD", 402, 0x074B8087},
	{"VGADICT", 1024, 0x28687D1A},
	{"VGAGRAPH", 302374, 0xE1D6765F},
	{"VGAHEAD", 477, 0x53B4B582},
	{"VSWAP", 1545400, 0x97FE030D},
	{NULL, 0, 0}
};

static const BE_GameFileDetails_T apogee12[] =
{
	{"AUDIOHED", 1156, 0x056AC3DB},
	{"AUDIOT", 317049, 0x1585D305},
	{"GAMEMAPS", 150758, 0x1029C727},
	{"MAPHEAD", 402, 0x5E43CAEE},
	{"VGADICT", 1024, 0x28687D1A},
	{"VGAGRAPH", 302374, 0xE1D6765F},
	{"VGAHEAD", 477, 0x53B4B582},
	{"VSWAP", 1545400, 0x97FE030D},
...
static const BE_GameFileDetails_T apogee14[] =
{
	{"AUDIOHED", 1156, 0x2F3BCB70},
	{"AUDIOT", 320209, 0xC79EFD4C},
	{"GAMEMAPS", 150652, 0xADA5C827},
	{"MAPHEAD", 402, 0x015803E9},
	{"VGADICT", 1024, 0x0BE27100},
	{"VGAGRAPH", 334506, 0x6AA208A5},
	{"VGAHEAD", 486, 0xEDDD83F8},
	{"VSWAP", 1545400, 0x859E8FFC},
	{NULL, 0, 0}
};

static const BE_GameFileDetails_T id14[] =
{
	{"AUDIOHED", 1156, 0x2F3BCB70},
	{"AUDIOT", 320209, 0xC79EFD4C},
	{"GAMEMAPS", 150652, 0xADA5C827},
	{"MAPHEAD", 402, 0x015803E9},
	{"VGADICT", 1024, 0x8B4AD0F8},
	{"VGAGRAPH", 276096, 0xE0DF1EA0},
	{"VGAHEAD", 450, 0x27EE37C4},
	{"VSWAP", 1545400, 0x859E8FFC},
	{NULL, 0, 0}
};
...
int main(void)
{
	...
	//int size;
	FILE *src, *in, *out;
	const char *patchdir = NULL;

	int res = 0;
	for (i = 0; gt14[i].filename != NULL; i++)
	{
		// get the checksum
		snprintf(filename, sizeof(filename), "%s.%s", gt14[i].filename, W3D_EXT);
		...
		adler = crc32(src);
		fclose(src);
		if (adler == gt14[i].crc32)
		{
			printf("OK\n");
			continue;
		}
		else if (compare_crc(apogee11[i].crc32, adler))
		{
			printf("Apogee v1.1\n");
			patchdir = "apogee11";
		}
		else if (compare_crc(apogee12[i].crc32, adler))
		{
			printf("Apogee v1.2\n");
			patchdir = "apogee12";
		}
		else if (compare_crc(apogee14[i].crc32, adler))
		{
			printf("Apogee v1.4\n");
			patchdir = "apogee14";
		}
		else if (compare_crc(id14[i].crc32, adler))
		{
			printf("Mindscape v1.4\n");
			patchdir = "id14";
		}
		else
		{
		...
		snprintf(patchname, sizeof(patchname), "%s/%s.%s", patchdir, gt14[i].filename, XD3_EXT);
		...
		snprintf(newname, sizeof(newname), "%s.%s", gt14[i].filename, "NEW");
		...
		// BS6 -> OLD
		snprintf(oldname, sizeof(oldname), "%s.%s", gt14[i].filename, "OLD");
