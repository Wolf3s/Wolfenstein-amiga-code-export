...
/*    if (SoundPositioned)
    ...
    {
        SetSoundLoc(globalsoundx, globalsoundy);
        SD_SetPosition(0, leftchannel, rightchannel);
    }
#else
...
void DrawPlayBorder (void)
{
	...
	const int vw = viewwidth;
#endif
    const int xl = screenWidth/2-vw/2;
    const int yl = (screenHeight-px*STATUSLINES-viewheight)/2;
    VWB_BarScaledCoord (xl,yl,vw,viewheight,0);

    if(xl != 0)
    {
        // Paint game view border lines
        VWB_BarScaledCoord(xl-px, yl-px, vw+px, px, 0);                      // upper border
        VWB_BarScaledCoord(xl, yl+viewheight, vw+px, px, bordercol-2);       // lower border
        VWB_BarScaledCoord(xl-px, yl-px, px, viewheight+px, 0);                     // left border
        VWB_BarScaledCoord(xl+vw, yl-px, px, viewheight+2*px, bordercol-2);  // right border
        VWB_BarScaledCoord(xl-px, yl+viewheight, px, px, bordercol-3);              // lower left highlight
    }
    else
    {//printf("%s bufferofs %d displayofs %d\n", __FUNCTION__, bufferofs, displayofs);
        // Just paint a lower border line
        VWB_BarScaledCoord(0, yl+viewheight, vw, px, bordercol-2);       // lower border
    }
}
...
void Died (void)
{
	...
	//printf("%s bufferofs %d displayofs %d\n", __FUNCTION__, bufferofs, displayofs);
#ifdef USE_HALFWIDTH
    VL_BarScaledCoord (viewscreenx,viewscreeny,viewwidth*2,viewheight,4);
#else
...
#ifdef USE_HALFWIDTH
    FizzleFade(screenBuffer,viewscreenx,viewscreeny,viewwidth*2,viewheight,70,false);
#else
