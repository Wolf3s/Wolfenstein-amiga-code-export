...
#define SWAP16LE(x) (uint16_t)(((uint16_t)(x) & 0xff) << 8 | ((uint16_t)(x) & 0xff00) >> 8)
#define SWAP32LE(x) (uint32_t)(((uint32_t)(x) & 0xff) << 24 | ((uint32_t)(x) & 0xff00) << 8 | \
...
#define curSurface NULL
#define screenBuffer NULL
...
#ifdef __AMIGA__
#define MAXDIST 16000 // TODO check if this is large enough
extern word heightnumdiv[];
#endif
...
#ifdef USE_AUTOMAP
extern  byte            mapvis[MAPSIZE][MAPSIZE];
#endif
...
extern  int16_t			tantoangle[TANTABLESIZE];
int16_t ATan2(fixed y, fixed x);
extern  int16_t			pixeltang[MAXVIEWWIDTH/2];
//extern  int wallheight[MAXVIEWWIDTH];
extern  word wallheight[MAXVIEWWIDTH];
#else
...
static inline fixed FixedMul(fixed a, fixed b)
{
	...
	//
	word b_lo = b&0xFFFF;
	longword result = b_lo*(a_as_unsigned>>16) + ((b_lo*(a_as_unsigned&0xFFFF))>>16);
	//
...
#ifdef __AMIGA__
//#define ISPOINTER(x) ((x) >= &objlist[0] && (x) < &objlist[MAXACTORS])
#define ISPOINTER(x) ((x) >= objlist)
#else
...
*/
#define ltoa(value, string, radix) itoa((value),(string),(radix))
#endif
...
#define READWORD(ptr) \
	({ \
		byte *src = (byte *)(ptr); \
		word val = src[0] | src[1] << 8; \
		(ptr) += 2; \
		val; \
	})

#define READLONGWORD(ptr) \
	({ \
		byte *src = (byte *)(ptr); \
		longword val = src[0] | src[1] << 8 | src[2] << 16 | src[3] << 24; \
		(ptr) += 4; \
		val; \
	})
