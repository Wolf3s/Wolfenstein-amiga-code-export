...
#elif defined(NUKED_OPL)
#include "opl3.h"
#elif defined(DOSBOX_OPL)
#include "dbopl.h"
#else
#ifdef __cplusplus
extern "C" {
...
#define OPL_NUM_OF_SAMPLES 1000 // same as the converter

static inline void YM3812UpdateOne(int which, INT16 *outbuf, int length)
{
	static int16_t buffer[OPL_NUM_OF_SAMPLES * 2];
	int i;

	if(length > OPL_NUM_OF_SAMPLES)
		Quit("%s length is too large: %d\n", __FUNCTION__, length);
		//length = OPL_NUM_OF_SAMPLES;

	OPL3_GenerateStream(&oplChip, buffer, length);
...
static inline void YM3812UpdateOne(int which, INT16 *outbuf, int length)
{
	...
	chip.GenerateBlock2( length, buffer );

	// Output is 32-bit stereo sound
	uint8_t *stream = (uint8_t *)outbuf;
	for(i = 0; i < length; i++)
	{
		// Scale volume
...
#ifdef __AMIGA__
    samplesPerMusicTick = (int)(((((int64_t)param_samplerate << 16) / 700) + (1 << 15)) >> 16);
    //samplesPerMusicTick = (int)floorf((float)param_samplerate / 700.0f + 0.5f);
#else
